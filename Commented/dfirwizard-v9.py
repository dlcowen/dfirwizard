#!/usr/bin/python
# Sample program or step 9 in becoming a DFIR Wizard!
...
import csv
import os
# re is Regular Expression, allows us to search for sequences of characters
import re
	...
	for entryObject in directoryObject:
		...
		try:
			...
			# If the file type is a regular file and it has a size the data can be hashed and written
				...
				# Checking to see if the file we're on matches the passed in search pattern, if not skip it and move on
				...
				# Check to see if the flag was set to extract the file
				if args.extract == True:
					# If the output directory doesn't exist in the folder running the script make it
					if not os.path.exists(outputPath):
						os.makedirs(outputPath)
					# Open a new file to write to and copy the data into this file
...
argparser.add_argument(
			...
			default='.*',
			required=False,
			help='Specify parameter e.g. *.lnk'
		)
...
dirPath = args.path
# Check to see if a search term was passed in, if so output it to the console
if not args.search == '.*':
